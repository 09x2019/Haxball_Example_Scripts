...
var JMap1 = JSON.parse(Map1); //These ones are for reaching the map names.
...
var room = HBInit({roomName:"TEST",noPlayer:true,public:false,maxPlayers:30});
...
function getVotesAndAnnounceResults(){
    ...
    room.sendAnnouncement(`Harita oylama oturumu başladı! ${VoteTimeout/1000} saniye içinde oylama bitecektir.`,null,0x00FFFF,"bold",2);
    GetVotesAndAnnounceResults_Timeout_1 = setTimeout(function(){
	room.sendAnnouncement(`Harita oylama oturumu sona erdi! İşte sonuçlar:`,null,0x00FFFF,"bold",2);
	for(var Map in Maps){
	...
	if(MaxVotedMaps.length == 0){
	    ...
	    admins.forEach(a => {
		room.sendAnnouncement("Hata! Bir şeyler yanlış gitti! Lütfen bir haritayı manuel olarak açın.",0xFFFF00,a.id,"bold",2)
	    });
	    nonadmins.forEach(n => {
		room.sendAnnouncement("Harita bir yönetici tarafından yüklenecektir. Lütfen bekleyin...",0xFFFF00,n.id,"bold",2)
	    });
	    ...
	    room.sendAnnouncement(`${MaxVotedMaps[0].Name} oylama sonucunda yüklendi. İyi oyunlar!`,null,0x00FF00,"bold",2);
	}
	else{
	    ...
	    room.sendAnnouncement(`${MaxVotedMaps[randomInt].Name} oylama oturumu sonucunda maksimum oylanan haritalar arasında rastgele yüklendi. İyi oyunlar!`,null,0x00FF00,"bold",2);
	}
    },MapStartWaitTimeout);
...
function voteMap(player,message){ //Command example: !vote 1
	...
	room.sendAnnouncement(`${player.name} oy verdi ${Maps[mapToVote-1].Name}`,null,0x00FF00,"bold",2);
    }
    else{
	if(VotedPlayers.includes(player.name) == true){
	    room.sendAnnouncement("Zaten bir harita için oy verdiniz! Lütfen oylama oturumunun bitmesini bekleyin.",player.id,0xFF0000,"bold",2); //Duplicated votes are not allowed.
	}
	else{
	    VotedPlayers.push(player.name);
	    Maps[mapToVote-1].Votes++;
...
room.onPlayerChat = function(player,message){
	...
	if(message.split(" ")[0] == VoteCommand){
	    ...
	    room.sendAnnouncement("Lütfen oylama oturumunun bitmesini bekleyin.",player.id,0xFFFF00,"bold",2);
	    admins.forEach(a => {
